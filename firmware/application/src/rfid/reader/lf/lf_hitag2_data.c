...
/**
 * Send a single bit using BPLM (Bi-Phase Mark) encoding
 * BPLM: Always transition at start, bit 1 has additional mid-bit transition
 * 
 * On ChameleonUltra, we use field modulation (like T55xx):
 * - Keep field ON for bit duration (simpler than ON/OFF transitions)
 * - Use gaps between bits for synchronization
 * 
 * @param bit The bit value (0 or 1)
 */
static void hitag2_send_bit(uint8_t bit) {
    // Keep field on for bit duration
    // Bit 0: 160μs, Bit 1: 240μs
    if (bit & 0x01) {
        bsp_delay_us(HITAG_T_1_US);
    } else {
        bsp_delay_us(HITAG_T_0_US);
    }
    
    // Small gap between bits (field stays on, just timing marker)
    // In real BPLM, this would be transitions, but we keep it simple
    bsp_delay_us(HITAG_T_GAP_US);
}
...
bool hitag2_read(uint8_t *data, uint32_t timeout_ms) {
    NRF_LOG_INFO("Hitag2 RTF protocol starting...");
    ...
    // Step 3: Send START_AUTH command (5 bits: 11000)
    NRF_LOG_INFO("Sending START_AUTH...");
    ...
    if (!ok) {
        ...
        NRF_LOG_INFO("Note: Full BPLM encoding not yet implemented");
        NRF_LOG_INFO("Current: Simplified field modulation");
        NRF_LOG_INFO("Needed: Proper ON/OFF transitions for BPLM");
    }
